		...
		const std::vector<LandmarkObs> &observations, const Map &map_landmarks) 
{
	...
	for(int p = 0; p < num_particles; p++)
	{
		...
		vector<double> sence_x;
		vector<double> sence_y;
		...
		{
			double close_dis = sensor_range;
			unsigned int association = 0;
			...
			for(unsigned int j = 0; j < map_landmarks.landmark_list.size(); j++)
			{
				...
				double cal_dist = sqrt(pow(trans_observations[i].x - landmark_x, 2.0) + pow(trans_observations[i].y - landmark_y, 2.0));
				if(close_dis >= cal_dist)
				{
					close_dis = cal_dist;
					association = j;
				}
			...
			double means_x = trans_observations[i].x;
			double means_y = trans_observations[i].y;
			...
			long double multipler = gauss_norm*exp(-(pow(means_x-mu_x,2)/(2*std_landmark[0]*std_landmark[0])+pow(means_y-mu_y,2)/(2*std_landmark[1]*std_landmark[1])));
